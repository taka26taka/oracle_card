...
const buildPrompt = (cardName, themeLabel, deepFocusLabel) => {
  const contextLines = [];
...
export const generateOracleMessage = async ({ cardName, theme, deepFocus, apiKey }) => {
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      model: DEFAULT_MODEL,
      input: buildPrompt(cardName, themeLabel, deepFocusLabel),
      temperature: 0.9,
