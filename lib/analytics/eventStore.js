...
const checkoutClickAttempts = new Map();
let loaded = false;
...
const rebuildAttemptIndexes = () => {
  ...
  for (const event of store) {
    ...
    if (event.event === EVENT_NAMES.PREMIUM_CHECKOUT_CLICKED) {
      checkoutClickAttempts.set(attemptId, new Date(event.ts).getTime());
    }
...
export const recordEvent = (body) => {
  ...
  if (!ALLOWED_EVENTS.has(payload.event)) return { recorded: false, reason: "invalid_payload" };

  const attemptId = payload?.meta?.attemptId;
  if (payload.event === EVENT_NAMES.PREMIUM_CHECKOUT_CLICKED && attemptId) {
    checkoutClickAttempts.set(attemptId, new Date(payload.ts).getTime());
  }

  if (payload.event === EVENT_NAMES.PURCHASE_COMPLETED && attemptId) {
    const checkoutTs = checkoutClickAttempts.get(attemptId);
    if (!checkoutTs) {
      return { recorded: false, reason: "invalid_purchase_attempt" };
    }
    if (Date.now() - checkoutTs > CHECKOUT_ATTEMPT_TTL_MS) {
...
export const hasCompletedPurchaseAttempt = (attemptId) => {
  ...
  return purchaseAttempts.has(attemptId);
};
