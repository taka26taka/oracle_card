...
const createDatabase = () => {
  ...
  instance.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_events_experiment_variant_date ON events(experiment_id, variant, date_key);
  `);

  // Forward-compatible migration for existing DB files.
  const columns = instance.prepare("PRAGMA table_info(events)").all().map((row) => row.name);
  if (!columns.includes("experiment_id")) {
    instance.exec("ALTER TABLE events ADD COLUMN experiment_id TEXT");
  }
  if (!columns.includes("variant")) {
    instance.exec("ALTER TABLE events ADD COLUMN variant TEXT");
  }
  return instance;
};
